...
@Client.on_message(command("STATS_COMMAND") & ~BANNED_USERS)
@language
async def stats_global(client, message: Message, _):
    ...
    await message.reply_photo(
        ...
        caption=_["gstats_11"].format(mention),
        reply_markup=upl,
    )


@Client.on_message(command("GSTATS_COMMAND") & ~BANNED_USERS)
@language
async def gstats_global(client, message: Message, _):
    ...
    user = await client.get_me()
    mention = user.mention
    final = f"Top played Tracks on  {mention}\n\n**Title:** {title}\n\nPlayed** {co} **times"
...
@Client.on_callback_query(filters.regex("GetStatsNow") & ~BANNED_USERS)
@languageCB
async def top_users_ten(client, CallbackQuery: CallbackQuery, _):
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                extract = (
                    (await client.get_users(items)).first_name
                    if what == "Users"
                    else (await client.get_chat(items)).title
                )
                if extract is None:
                    continue
        ...
        mention = user.mention
        temp = (
            _["gstats_5"].format(limit, mention)
            if what == "Chats"
            else _["gstats_6"].format(limit, mention)
        )
        msg = temp + msg
...
async def back_buttons(client, CallbackQuery, _):
    ...
    if command == "GlobalStats":
        ...
        med = InputMediaPhoto(
            media=config.GLOBAL_IMG_URL,
            caption=_["gstats_10"].format(mention),
        )
        try:
            await CallbackQuery.edit_message_media(media=med, reply_markup=upl)
        except MessageIdInvalid:
