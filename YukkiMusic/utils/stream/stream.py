...
async def stream(
    ...
    usera = await app.get_me()
    BOT_USERNAME = usera.mention
    if not result:
        return
    if video:
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                await Yukki.join_call(
                    chat_id, original_chat_id, file_path, video=status, image=thumbnail,app=app
                )
                await put_queue(
                    chat_id,
                    original_chat_id,
        ...
        if streamtype == "saavn_track":
            ...
            if await is_active_chat(chat_id):
                ...
                if not forceplay:
                    db[chat_id] = []
                await Yukki.join_call(chat_id, original_chat_id, file_path, video=None,app=app)
                await put_queue(
                    chat_id,
                    original_chat_id,
                    file_path,
            ...
            for search in result:
                ...
                if await is_active_chat(chat_id):
                    ...
                    if not forceplay:
                        db[chat_id] = []
                    await Yukki.join_call(
                        chat_id, original_chat_id, file_path, video=None,app=app
                    )
                    await put_queue(
                        chat_id,
                        original_chat_id,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Yukki.join_call(chat_id, original_chat_id, file_path, video=status,app=app)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
